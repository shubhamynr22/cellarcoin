...
contract NftMarketplace is ERC721 {
    ...
    //@param _to - The address to which the  mined NFT has to be sent
    //@param _tokenURI the json file url containing the details
    //@param _timePeriod the unix timestamp for the time of the mint
    //@param _externalLink the The link to the website
    //@param _description The description that is in the JSON file
    function nftMint(
        address _to,
        string memory _tokenURI,
        string memory _mintName,
    ...
    //@param tokenid The id of the MFT which has to be put up for sale
    //@price price The price for which it has to be sold in wei.
    function placeNFTForSale(uint256 tokenid, uint256 price) external {
    ...
    //@param token: Address of the ERC20 token with which the payment is done. 0x0 incase of native token
    function buynftwithERC(uint256 tokenid, address token) external payable {
    ...
    //@param _tokenURI the updated token url link which contains the metadata
